(function () {
  ...
  function renderHome() {
    ...
    tab.innerHTML = `
      ...
      ${planned.length ? `<div class="section-title" style="margin-top:8px"><div><h3>计划收集</h3><p class="muted">这些车辆高亮置顶，但不会进入比赛和成绩区。</p></div></div><div class="home-cars-wrap"><div class="car-grid planned-row">${planned.map((entry) => `<article class="car-card planned-card"><div class="car-top">${cardMedia(entry)}<div><h3>${entry.character}</h3><div class="muted">${entry.vehicle}${entry.glider ? ` + ${entry.glider}` : ""}</div></div></div><div class="tags"><span class="tag collected">计划收集</span><span class="tag">等级分 ${formatRating(entry.rating)}</span></div><div class="muted small">${entry.collectionNote || "暂无收藏备注"}</div></article>`).join("")}</div></div>` : ""}
      ${collected.length ? `<div class="section-title" style="margin-top:22px"><div><h3>已收藏</h3><p class="muted">这些车辆可进入比赛区和成绩区。</p></div></div><div class="home-cars-wrap"><div class="car-grid">${collected.map((entry) => `<article class="car-card"><div class="car-top">${cardMedia(entry)}<div><h3>${entry.character}</h3><div class="muted">${entry.vehicle}${entry.glider ? ` + ${entry.glider}` : ""}</div></div></div><div class="tags"><span class="tag collected">${entry.type === "glider" ? "滑翔伞车型" : "普通车型"}</span><span class="tag">等级分 ${formatRating(entry.rating)}</span><span class="tag">收藏于 ${formatDateOnly(entry.collectedAt)}</span></div><div class="muted small">${entry.collectionNote || "暂无收藏备注"}</div></article>`).join("")}</div></div>` : ""}
    `;
  }
